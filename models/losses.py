...
def reg_loss(coeffs_dict, opt=None):
    ...
    creg_loss = 0
    if opt.w_id and 'id' in coeffs_dict:
        creg_loss += w_id * torch.sum(coeffs_dict['id'] ** 2)

    if opt.w_exp and 'exp' in coeffs_dict:
        creg_loss += w_exp * torch.sum(coeffs_dict['exp'] ** 2)

    if opt.w_tex and 'tex' in coeffs_dict:
        creg_loss += w_tex * torch.sum(coeffs_dict['tex'] ** 2)
    # creg_loss = w_id * torch.sum(coeffs_dict['id'] ** 2) +  \
...
def reflectance_loss(texture, mask):
    ...
    # print(mask.shape, texture.shape)
