...
from torch.nn import Module, ModuleList, Linear, ELU, init, BatchNorm1d
...
class AutoEncoder(Module):
    def __init__(self, feature_channels: int, latent_channels: int, a: list, d: list, u: list,
                 enc_k: int = 6, dec_k: int = 6, lambda_max: float = 2.3,
        ...
        self.encoder = Encoder(self.filters, edge_index_list, down_transform_list, latent_channels, enc_k,
                               lambda_max=lambda_max, conv=enc_conv).to(device)
        self.decoder = Decoder(self.filters, edge_index_list, up_transform_list, latent_channels, dec_k,
...
class VariationalAutoEncoder(Module):
    def __init__(self, feature_channels: int, latent_channels: int, a: list, d: list, u: list,
        ...
        self.decoder = Decoder(self.filters, edge_index_list, up_transform_list, latent_channels,
